...
surveysRouter.get("/surveys/:surveyId", async (req, res) => {
  try {
    ...
    let [rawQuestions] = await pool.query(
      `SELECT 
      Question.*, 
      SurveyQuestion.id AS surveyQuestionId, 
      IF(surveyId IS NULL, FALSE, TRUE) AS isActive 
      FROM Question 
      LEFT JOIN SurveyQuestion ON Question.id = SurveyQuestion.questionId 
      AND surveyId = ? 
    ...
    let questions = z
      .object({
        id: z.number(),
        surveyQuestionId: z.number().nullable(),
        title: z.string(),
